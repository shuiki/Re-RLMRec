...
class DCCF_gene(BaseModel):
    def __init__(self, data_handler):
        ...
        # semantic-embeddings
        self.usrprf_embeds = nn.Parameter(torch.tensor(configs['usrprf_embeds']).float())
        self.itmprf_embeds = nn.Parameter(torch.tensor(configs['itmprf_embeds']).float())

        # weight params
        self.w_uu = nn.Parameter(torch.tensor(0.8))
        self.w_ii = nn.Parameter(torch.tensor(0.8))

        # pos_samples for contrastive adapter
        self.usr_pos_sample_idx = torch.tensor(configs['usr_pos_samples_idx']).T[1].long()
        self.itm_pos_sample_idx = torch.tensor(configs['itm_pos_samples_idx']).T[1].long()
    ...
    def _reconstruction(self, embeds, seeds):
        ...
        prf_embeds = torch.concat([self.usrprf_embeds, self.itmprf_embeds], dim=0)[seeds]
